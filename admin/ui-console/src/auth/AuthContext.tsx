...
import { useTheme, type Mode } from "../theme/ThemeContext";
...
function applyServerTheme(theme: string | null, setMode: (m: Mode, opts?: { persist?: boolean }) => void) {
  if (theme === "light" || theme === "dark") setMode(theme, { persist: false });
}
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((result) => {
        if (cancelled) return;
        setProfile(result);
        if (result) applyServerTheme(result.theme, setMode);
      })
      .finally(() => {
  ...
  const login = async (identifier: string, password: string, sessionType: "Fixed" | "Extended") => {
    ...
    setProfile(result);
    applyServerTheme(result.theme, setMode);
  };
